...
void vendor_screen(struct State *state, struct Cheat cheats_list[]) {
    ...
    while(in_shop) {
        // update prices
        int scanner_price = SHOP_SCANNER_MULT * ((state->scanner_lvl + 1) * (state->scanner_lvl + 1));
        int cheat_slots_price = SHOP_CHEAT_SLOTS_MULT * (state->cheat_slots * state->cheat_slots);
