...
void match_print_score_bar(int total) {
    ...
    enum Color color = total > GOAL_NUM ? CRL2 : CRL5;
    color = total == GOAL_NUM ? CRL3 : color;
...
void match_print_opponent(struct State *state, struct Match *match) {
    ...
    enum Color unknown_color = CRL1;
    enum Color yes_color = CRL2;
    enum Color no_color = CRL3;
...
void match_start(struct State *state, struct Match *match) {
    ...
    print_div();
    print_subtitle(CRL2, "Match Started");
...
void match_end_screen(struct Match *match, struct State *state, struct Cheat cheats_list[], enum Match_Res match_res, char msg[]) { 
    print_subtitle(CRL2, "Match Ended");
    input_to_continue();
    // Update gold
    int gold_delta = 0;
    ...
    // Draw to screen
    enum Color result_colors[5] = {CRL2, CRL4, CRL5, CRL3, CRL6};
    ...
    printfc(result_color, "%s\n", msg);
    match_print_opponent(state, match);
    if (gold_delta == 0) printfc(CRL1, "// Gained no gold\n");
    if (gold_delta > 0) printfc(CRL4, "++ Gained %d gold\n", gold_delta);
    if (gold_delta < 0) printfc(CRL2, "-- Lost %d gold\n", -gold_delta);
    if (match->opponent.trait_high_stakes) printfc(CRL4, "(Match was HI-STAKES)\n");
    state_print_status(state, 1);
    // All results greater than DRAW are wins
    if (match_res > MATCH_RES_DRAW) printfc(CRL4, "You got a loot box for winning!");
    input_to_continue();
    // Get new cheat
