...
void welcome_screen(struct State *state) {
    ...
    for (int i = 0; i < strlen(state->username); i++) {
        seed_sum += state->username[i];
    }
    for (int i = 0; i < strlen(state->password); i++) {
        seed_sum += state->password[i];
    }
    // Set the random seed
    srand(seed_sum);
    clear_screen("LOGIN ACCEPTED");
    printfc(CLR1, "srng-hash-");
    for (int i = 0; i < 256; i++) {
        printfc(rand() % 8, "%c", rand() % 26 + 65);
        if (i % 32 == 31) printfc(CLR1, "\n          ");
    }
    printfc(CLR1, "\n            -- END SRNG HASH --\n");
    input_to_continue();
    clear_screen("WELCOME");
    print_logo();
    printfc(CLR1, "Hello %s (#%d)!\n", state->username, seed_sum);
...
void intro_screen() {
    ...
    printfc(CLR6, " %c, %c", CFG_CMD_USE_CHEAT_1, CFG_CMD_USE_CHEAT_2);
